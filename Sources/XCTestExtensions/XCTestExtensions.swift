...
public func XCTAssertEqual(_ string: String, _ expected: String, ignoringWhitespace: Bool = false, file: StaticString = #file, line fileLine: UInt = #line) {
    if string != expected {
        ...
        if lineCount < expectedLineCount {
            XCTFail("\(expectedLineCount - lineCount) lines missing from string.", file: file, line: fileLine)
        } else if lineCount > expectedLineCount {
            XCTFail("\(lineCount - expectedLineCount) extra lines in string.", file: file, line: fileLine)
        }
    }
}
...
public func XCTAssertEqualIgnoringWhitespace(_ string: String, _ expected: String, file: StaticString = #file, line: UInt = #line) {
    XCTAssertEqual(string, expected, ignoringWhitespace: true, file: file, line: line)
}
